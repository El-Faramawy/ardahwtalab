...
function myrules($rule)
{
    $role = Auth::user()->role_id;
    $role = App\Models\Roles::find($role);
    $roles = explode(',', $role->roles);
    return in_array($rule, $roles) ? true : false;
}
...
function ImageResize($photo)
{
    ...
    return $imagee->cache(function ($image) use ($photo) {
        $image->make($photo)->resize(300, 200)->greyscale();
    });
}
...
function get_dept_details($dept)
{
    $info = new \App\Models\Advs();
    $super_parent = $dept->super_parent;
    $props = \App\Models\Props::where(['dept_id' => $super_parent->id, 'parent' => NULL])
    ...
    return view('ajax.details', ['props' => $props, 'dept' => $dept, 'info' => $info]);
}
...
/*  ====================================================================================================  */
function deptLayout($dept) {

        if(count($dept->childs) > 0) {
            foreach($dept->childs as $child) {

                $retrun = [" <div class=\"main-xs-nav mo-droplinks level2\" data-id=\"{{ $i }}\">
                                <ul class=\"fixall nav-links list-unstyled\">
                                    @foreach($child->childs as $child)
                                        <li class=\"navlink-li fixall megaxs-link\">

                                        @if($child->childs()->count())
                                            <a href=\"#!\"
                                            class=\"navlink fixall\">{{ $child->name }}</a>
                                        @else
                                            <a href=\"{{ $child->childs()->count() ? $child->link : $child->link }}\"
                                                class=\"navlink fixall\">{{ $child->name }}</a>
                                        @endif

                                        " . $this->deptLayout($child) . "

                                    @endforeach
                                </ul>
                            </div> "];

                $retrun[] = $this->deptLayout($child);
            }
...
function drawMenuDeskTop($dept) {
    $layout = '';
    $layout .= '<li class="fixall navlink-li mega-menu-btn">';
    $layout .= '<a href="#!" class="fixall navlink">'.$dept->name.'</a>';
    if($dept->childs()->count()) {
        $layout .= '<div class="megamenu-cont">';
        $i = 1;
            foreach($dept->childs as $child) {
                if($i % 6 == 1) {
                    $layout .= '<ul class="links">';
                    $layout .= TreeDrawMenuDeskTop($child);
                    $layout .= '</ul>';
                } else {
                    $layout .= TreeDrawMenuDeskTop($child);
                }
